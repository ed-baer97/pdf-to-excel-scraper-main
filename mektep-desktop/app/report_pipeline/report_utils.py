...
def _load_criteria_context(batch_subdir: Path) -> dict:
    ctx_file = batch_subdir / "criteria_context.json"
...
def visible_soch_column(batch_subdir: Path) -> bool:
    ...
    ctx = _load_criteria_context(batch_subdir)
    if "visible_soch_column" in ctx:
        return bool(ctx["visible_soch_column"])
    return False


def has_grade_summary_columns(batch_subdir: Path) -> bool:
    """В видимой таблице есть колонки «Сумма%» и «Оценка»."""
    ctx = _load_criteria_context(batch_subdir)
    if "visible_grade_summary_columns" in ctx:
        return bool(ctx["visible_grade_summary_columns"])
    if "has_grade_summary_columns" in ctx:
