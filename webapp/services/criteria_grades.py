...
def collect_subject_tables_for_class(
    ...
    """Данные по предметам класса за период (таблица, payload, учитель)."""
    sheets: list[dict[str, Any]] = []
    seen: set[str] = set()
    for report in reports:
        if report.class_name != class_name:
            continue
        ...
        if is_final_period(period_number):
            ...
            continue
        table = table_for_period_payload(period_number, payload)
        if not table:
            continue
        seen.add(subj)
        sheets.append(
    ...
    sheets.sort(key=lambda x: kazakh_sort_key(x["subject"]))
    return sheets
