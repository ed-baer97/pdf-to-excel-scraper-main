...
# Родитель пакета mektep_core в sys.path — для прямого запуска файла (EXE / mektep-desktop).
_PACKAGE_PARENT = Path(__file__).resolve().parent.parent
_package_parent_str = str(_PACKAGE_PARENT)
if _package_parent_str not in sys.path:
    sys.path.insert(0, _package_parent_str)

from mektep_core.grade_table_signals import analyze_visible_table_headers, detect_grade_summary_columns
...
# Импорт логгера
try:
    from mektep_core.scraper_logger import (
        init_logger,
        get_logger,
        log_stage,
        log_info,
...
def _resolve_template_path(name: str) -> Path:
    ...
    # Project root (parent of the mektep_core package)
...
def run(headless: bool, out_dir: Path, slow_mo_ms: int) -> int:
    ...
    if expected_iin:
        try:
            from mektep_core.iin_utils import normalize_kz_iin as _norm_iin
        except ImportError:
            def _norm_iin(s):
                d = re.sub(r"\D", "", s or "")
    ...
    with sync_playwright() as p:
        ...
        def process_one(selected: dict, batch_subdir: Path) -> None:
            ...
            # Build Excel report
            try:
                from mektep_core.build_report import build_report
                ...
                if template_path.exists():
                    ...
                    # Build Word report
                    try:
                        from mektep_core.build_word_report import build_word_report
